...
app.use(function (req, res, next) {
  // Middleware para permitir consumo de APIS localmente.
...
app.get("/api/items", (req, res) => {
  ...
  (async () => {
    try {
      const externalAPI = await got(
        `https://api.mercadolibre.com/sites/MLA/search?q=${query}`
      ); // Consumimos el endpoint en el API de Meli con el parámetro solicitado.
      const body = JSON.parse(externalAPI.body); // Obtenemos la respuesta del servicio.
      let bodyAux = {
        // Construimos el array deseado con la información obtenida del API.
        author: {
          name: "Julio",
          lastname: "Arroyave",
        },
...
app.get("/api/items/:id", (req, res) => {
  ...
  (async () => {
    try {
      ...
      const extItemDescrAPI = await got(
        `https://api.mercadolibre.com/items/${id}/description`
      ); // Consumimos el endpoint de Meli para obtener la descripción del item específico.
      ...
      let bodyAux = {
        // Construimos el body con el formato específico.
        author: {
          name: "Julio",
          lastname: "Arroyave",
        },
